...
Shader "TextMeshPro/Mobile/Billboard/Distance Field" {
...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
			...
			float3 transformScale = float3(length(transposed[0]), length(transposed[1]), length(transposed[2]));

			float3x3 newBasis = float3x3(
			float3(c * transformScale.x, 0, s * transformScale.z),
			float3(0, transformScale.y, 0),
			float3(-s * transformScale.x, 0, c * transformScale.z)
			...
			float4 vPosition = mul(UNITY_MATRIX_VP, mul(objectToWorld, input.vertex));
